...
def CAMB(camb_dict):
    ...
    camb_dict["output_root"] = localtime
    os.chdir('camb')
    os.system("cp HMF_params.ini params_"+localtime+'.ini')
    SetParameter('params_'+localtime+'.ini', camb_dict)
    os.system('module load gfortran')
    os.system('./camb params_'+localtime+'.ini')
    os.system('rm params_'+localtime+'.ini')
    os.chdir('..')
